const { listItems } = require("../model/dao/catalogDao");
const Items = require("../model/models/items");
...
class CatalogController {
    ...
    static async insert(req, res) {
        try {
            let np = new Items();
            if (req.body.Descrizione) np.setDescrizione(req.body.Descrizione);
            if (req.body.Prezzo) np.setPrezzo(req.body.Prezzo);
            if (req.body.SKU) np.setSKU(req.body.SKU);
            //if (req.body.IsDeleted !== null) np.setIsDeleted(req.body.IsDeleted);

            await np.save();
            res.status(200).send("Catalogo aggiornato");
        } catch (e) {
